...
2. Acceptance: capture requires the `acceptance` in-artifact (`workspace:accepted`), written only by the developer after they exercised the built behavior and it matches intent. Verify it: `bash ${CLAUDE_PLUGIN_ROOT}/bin/workspace.sh verify-acceptance --repo .` Exit 5 means no marker - refuse and tell the developer to record acceptance themselves: `bash ${CLAUDE_PLUGIN_ROOT}/bin/workspace.sh accept --repo .` A stale digest mismatch (exit 1) means the diff moved after acceptance - refuse and tell the developer to exercise the current behavior and re-accept with that same accept command; the marker must digest the diff it attests. Never run accept on the developer's behalf - acceptance is a human act.
